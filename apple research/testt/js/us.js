...
optimizelyCode = function (data) {
    ...
    optly.Cleanse.finish = function () {
        if (optly.Cleanse.running) {
            ...
            var log = window.location.hash.indexOf(param) !== -1 || window.location.search.indexOf(param) !== -1;
    ...
    optly.Cleanse.log = function (action, typeName, propertyName) {
        if (typeName) {
            ...
            optly.Cleanse.logs.push("Optimizely / Info / Cleanse / " + action + ": " + typeName + "." + propertyName);
        }
        else {
    ...
    optly.Cleanse.start = function () {
        ...
        if (hostname.indexOf("optimizely") !== -1 && hostname.indexOf("edit") === -1 && hostname.indexOf("preview") === -1 && hostname.indexOf("test") === -1) {
            return;
        }
        if (optly.Cleanse.running) {
        ...
        optly.Cleanse.each(optly.Cleanse.types, function (typeName, type) {
            ...
            optly.Cleanse.each(type.prototype, function (propertyName, property, getter, setter) {
                if (optly.nativity.isNativeFunction(property) || optly.nativity.isNativeFunction(getter) || optly.nativity.isNativeFunction(setter)) {
    ...
    optly.murmur3.hash32 = function (data, seed) {
        ...
        for (var i = 0; i < roundedEnd; i += 4) {
            k1 = (data.charCodeAt(i) & 0xff) | ((data.charCodeAt(i + 1) & 0xff) << 8) | ((data.charCodeAt(i + 2) & 0xff) << 16) | ((data.charCodeAt(i + 3) & 0xff) << 24);
            k1 = mul32(k1, c1);
    ...
    tracker.addLogEventFromAPI = function (name, revenueOrOptions) {
        ...
        if (customRevenueGoals && name in customRevenueGoals && isNumberLike(customRevenueGoals[name])) {
        ...
        if (revenueOrOptions && isNumberLike(revenueOrOptions)) {
            ...
            options = {
                revenue: Number(revenueOrOptions)
            };
        }
        else if (revenueOrOptions && revenueOrOptions['revenue']) {
            options = /** @type {Object} */ {
                revenue: revenueOrOptions['revenue']
            }
        }
    ...
    tracker.logEvents = [];
    /** @private */
    tracker.shouldSendLogEvents = false;
    /**
    ...
    , "src=js"
        ...
        each(tracker.logEvents, function (event) {
            ...
            each(eventExperimentSets, function (eventExperimentSet) {
                ...
                individualQuerySubstrings.push(eventQueryParameters.concat(eventExperimentSet.extraParameters).concat([
        ...
        for (var i = 0, nEvents = eventsToSend.length; i < nEvents; i++) {
            ...
            tracker.makeLogRequest(queryCommonSubstring + "&" + eventQuery, function () {
    ...
    tracker.sendVerificationRequest = function () {
            ...
            var fullUrl = "//" + webHost + "/account/snippet_installed?project_id=" + data.getProjectId() + "&wxhr=true";
    ...
    tracker.setPendingLogEventsCookie = function (events) {
        ...
        cookie.set(PENDING_LOG_EVENTS_COOKIE_NAME, value, PENDING_LOG_EVENTS_COOKIE_AGE);
    };
    /**
    ...
    tracker.updateCustomEventList = function (eventName) {
        ...
        var eventArray = customEventList[userId] || (customEventList[userId] = []);
    ...
    data.getEnabledExperimentIds = function () {
        if (!data.cachedEnabledExperimentIds) {
            data.cachedEnabledExperimentIds = filter(data.getExperimentIds(), data.isExperimentEnabled);
        }
    ...
    data.getProjectClickGoalsWithCustomTargeting = function () {
        var projectClickGoalsWithCustomTargeting = filter(data.getClickGoals(), function (goal) {
            if (goal['experiments']) {
    ...
    data.getExperimentEvents = function (experimentId) {
        ...
        var projectClickGoalsTrackingThisExperiment = filter(data.getClickGoals(), function (goal) {
            if ('experiments' in goal) {
    ...
    data.getSiteCatalystEVar = function (experimentId) {
        return data.getExperimentProperty(experimentId, "site_catalyst_evar") || null;
    };
    /**
     * @param {string} experimentId
     * @return {?number}
    ...
    data.getSiteCatalystProp = function (experimentId) {
        return data.getExperimentProperty(experimentId, "site_catalyst_prop") || null;
    };
    /**
    ...
    data.getGeotargetingProperty = function (experimentOrSegmentId) {
        ...
        return data.getExperimentProperty(experimentOrSegmentId, property) || data.getSegmentProperty(experimentOrSegmentId, property);
    };
    /**
     * @private
     * @param {string} experimentId
    ...
    preview_loader.loadPreview = function () {
        ...
        if (!window['optimizelyPreview']) {
            ...
            loadScript(
      [preview_loader.HOST, preview_loader.PATH, directive.previewScriptName, ".js"].join(""), // Attempt a synchronous load. Will fall back to asynchronous if that isn't possible.
                true);
    ...
    preview_loader.loadSplashScreen = function (splashScreenContent) {
        ...
        var splashScreenHtml = '<div id="optimizely-loading" style="position:absolute;top:0;right:0;left:0;bottom:0;background-color:white;opacity:0.9;z-index: 3271000;">' + '<h2 style="color:#9a9a9a;top:40%;position:absolute;font-size:2.25em;text-align:center;width:100%;font-family:\'Lucida Grande\',sans-serif;">';
    ...
    integrator.activateSiteCatalyst = function (options) {
        ...
        each(integrator.getRelevantVariationIds(), function (variationId) {
            var experimentId = data.getVariationExperimentId(variationId);
            var keyValue = integrator.makeSendableNames(experimentId, variationId, 100, 100, 25, true);
                    ...
                    if (context['inSingleRecordingScope']) {
                        ...
                        for (var key in context['fields']) {
                            ...
                            if (!integrator.getRedirectURL(context['fields'][key])) {
                                window['optimizely'].push(["activate", key, context['fields'][key], {
                                    'force': true
                                }]);
                            }
                            else {
    ...
    integrator.recordClickTale = function (options) {
        ...
        if (typeof window['ClickTaleField'] == 'function') {
            each(integrator.getRelevantVariationIds(), function (variationId) {
                ...
                var value = keyValue.key + ": " + keyValue.value + " (x" + experimentId + "=" + variationId + ")";
    ...
    integrator.fixGoogleAnalyticsReferrer = function () {
        ...
        if (referrer !== null) {
            try {
                ...
                log("Integrator", "Error setting Google Analytics referrer: %s", exception);
            }
        }
    };
    /**
    ...
    integrator.fixUniversalAnalyticsReferrer = ENABLED_UNIVERSAL_ANALYTICS ? function () {
        ...
        if (referrer !== null) {
            try {
                ...
                log("Integrator", "Error setting Universal Analytics referrer: %s", exception);
            }
        }
    } : goog.nullFunction;
    /**
    ...
    integrator.fixSiteCatalystReferrer = function (siteCatalyst) {
        ...
        if (referrer !== null && siteCatalyst) {
            try {
                ...
                log("Integrator", "Error setting SiteCatalyst referrer: %s", exception);
            }
        }
    };
    /**
        ...
        each(plan.getVariationIds(), function (variationId) {
            ...
            if (data.isExperimentEnabled(experimentId)) {
                ...
                if (arrayContains(activeExperimentIds, experimentId)) {
                    log("Integrator", "\"%s\" relevant because experiment active", variationName);
                    relevant = true;
                }
                if (relevant) {
    ...
    integrator.getSiteCatalystSVar = function () {
        return integrator.siteCatalystObject || (typeof window['s'] !== "undefined" ? window['s'] : null);
    };
    /**
     * @private
     * @param {string} variationId
    ...
    integrator.makeGoogleAnalyticsRequest = function (experimentId, variationId) {
        ...
        try {
            ...
            log("Integrator", "Calling _gaq._setCustomVar for slot %d and scope %d", slot, integrator._googleAnalyticsCustomVariableScope);
    ...
    integrator.activateUniversalAnalytics = ENABLED_UNIVERSAL_ANALYTICS ? function () {
        ...
        each(integrator.getRelevantVariationIds(), function (variationId) {
            ...
            if (data.isUniversalAnalyticsEnabled(experimentId)) {
                ...
                log('Integrator', 'Calling ua set dimension - ga(%sset, dimension%d, %s)', prefix, slot, value);
    ...
    integrator.makeATInternetRequest = ENABLED_AT_INTERNET ? function (experimentId, variationId) {
        ...
        try {
            log('Integrator', 'Sending AT Internet log call for account %s', acctNo);
     ...
     * @private
     */
    integrator.makeSendableNames = function (experimentId, variationId, expLength, varLength, mvtVarLength, makeClean) {
    ...
    api.activate = function (activateExperimentId, variationIdOrForcedActivation, options) {
        ...
        var preferredVariationId = (typeof variationIdOrForcedActivation === "number" || typeof variationIdOrForcedActivation === "string") ? String(variationIdOrForcedActivation) : null;
        // If second param is a boolean or options.force is true, enabled forced activation mode.
        var forcedActivation = ((variationIdOrForcedActivation === true) || (variationIdOrForcedActivation && variationIdOrForcedActivation['force'] === true) || (options && options['force'] === true));
        ...
        if (preferredVariationId) {
            try {
                ...
                api.bucketUser( /** @type {number|string} */ (activateExperimentId), preferredVariationId, doNotSendPageViewTracking);
            }
            catch (err) {
                timer.addEvent("API", "Error while activating experiment " + activateExperimentId + " for variation " + preferredVariationId + " -- proceeding without bucketing user.");
            }
        }
        ...
        each(experimentsToActivate, function (experimentId) {
            ...
            else if (forcedActivation || condition.passesTests(experimentId, {
                    "manualMode": true
    ...
    api.activateGeoDelayedExperiments = function (asyncInfo) {
        ...
        function activateDelayed() {
            ...
            each(api.geoDelayedExperiments, function (experimentId) {
                if (condition.passesTests(experimentId, {
                        "manualMode": true
                        , "objectType": "experiment"
                        , "visitor": api.visitor
                    })) {
    ...
    api.initialize = function (apiVisitor) {
        ...
        var globals = {
            ...
            , 'data': api.data, // Saves getElementsByClassName that hopefully hasn't been changed yet by
            // external code.
            ...
            extend(globalFunctions, {
                ...
                'verifyPreviewProject': function (projectId) {
                    if (data.getProjectId() !== projectId) {
                        ...
                        timer.addEvent('API', 'Preview projectId (' + data.getProjectId() + ') ' + 'does not match expected (' + projectId + '), disabling.');
                    }
                    else {
    ...
    /** @type Array.<string> */
    api.geoDelayedExperiments = [];
    /** @type Array.<string> */
    api.geoDelayedSegments = [];
    /** @private */
    api.variationIdsMap = {};
    /** @private */
    api.variationMap = {};
        ...
        each(data.getExperimentIds(), function (experimentId) {
            if (!evaluator.isInQueue(experimentId)) {
                if (data.isExperimentConditionallyActivated(experimentId) && data.isExperimentEnabled(experimentId)) {
                    ...
                    if (!data.isExperimentManuallyActivated(experimentId) && !arrayContains(evaluator.getEvaluatedExperimentIds(), experimentId)) {
                        api.addGeoDelayedExperiment(experimentId);
                    }
                }
    ...
    distributor.distributeExperiment = function (experimentId, forceIgnored) {
        ...
        if (enabledVariationIds.length === 0) {
            ...
            if (ignorePercentage > distributor.randomForIgnoring(experimentId)) {
                timer.addEvent("Distributor", "Permanently ignoring experiment " + experimentId + "(" + (ignorePercentage / 100) + "% likelihood)");
                plan.ignoreExperiment(experimentId);
                return false;
            }
            else {
                ...
                var randomIndex = distributor.pickRandomVariationIndex(experimentId, variationIds);
                var variationId = variationIds[randomIndex];
                timer.addEvent("Distributor", "Picked variation " + variationId + " [index " + randomIndex + " of " + enabledVariationIds.length + "]");
        ...
        if (!pollingType) {
            ...
            if (options['force'] || !condition.isReadyToTest(experimentId) || condition.passesTests(experimentId, testOptions)) {
                var pollForReady = function () {
                    if (condition.isReadyToTest(experimentId) && (condition.testCodeResult(null, {
                            'value': code
                        }) || conditionalCodeResult)) {
                        activationFunction();
                    }
                    else {
    ...
    distributor.randomForIgnoring = function (experimentId) {
        ...
        return optly.hashing.hashToInt(distributor.getBucketingId(), experimentId, optly.hashing.HashSeed.IGNORING, 10000);
    };
    /**
    ...
    distributor.randomForBucketing = function (experimentId) {
        ...
        // Other calls to distributor.experimentUserHash or .randomInt must use other seeds.
        return optly.hashing.experimentUserHash(distributor.getBucketingId(), experimentId, optly.hashing.HashSeed.BUCKETING);
    };
    /**
    ...
    distributor.getRandomBucketAsFallback = function (experimentId, numBuckets) {
        ...
        // Other calls to distributor.experimentUserHash or .randomInt must use other seeds.
        return optly.hashing.hashToInt(distributor.getBucketingId(), experimentId, optly.hashing.HashSeed.FALLBACK, numBuckets);
    };
    /**
     ...
     * @return {?number}
     */
    distributor.pickRandomVariationIndex = function (experimentId, possibleVariationIds) {
        var possibleWeights = [];
    ...
    distributor.preferMVTVariation = function (experimentId, variationPartial) {
        ...
        timer.addEvent("Distributor", "Preferring variation partial " + variationPartial + " of section " + sectionId + " of experiment " + experimentId);
    ...
    distributor.getPotentialPreferredMVTVariations = function (experimentId) {
        ...
        each(data.getEnabledVariationIds(experimentId), function (variationId) {
            if (distributor.containsPreferredMVTVariations(experimentId, variationId)) {
                variationIds.push(variationId);
            }
        });
        return variationIds;
    ...
    /** @private */
    distributor.preferredBucketUserMapping = {};
    ...
    query.process = function () {
        ...
        if (queryParameters['opt_out'] === "true" || queryParameters['opt_out'] === "false") {
            tracker.optOut(queryParameters['opt_out'] === "true");
        }
        directive.forceTracking = queryParameters['force_tracking'] === "true";
        if (queryParameters['disable'] === 'true' || queryParameters['opt_out'] === 'true' || cookie.get(OPT_OUT_COOKIE_NAME) === 'true') {
            directive.isEnabled = false;
        }
    ...
    redirect.setRedirectCookie = function (noRedirectCookie, variationId) {
        ...
        cookie.set(REDIRECT_COOKIE_NAME, (variationId || "unknown variation") + "|" + noRedirectCookieString, REDIRECT_COOKIE_SECONDS);
    };
    /**
    ...
    */
    ...
    segmenter.handleOfflineSegmentResponse = function (data) {
        ...
        for (segmentId in data.map) {
            log("Segmenter", "Offline segment request returned: %s, %s", segmentId, data.map[segmentId]);
    ...
    segmenter.tellPreviewAboutSegment = optly.client.preview.ENABLED ? function (segmentId, addOrRemove) {
        ...
        if (reason) {
            optly.client.preview.Logger.addEvent("Not " + addOrRemoveVerb + " visitor " + toOrFromPreposition + " segment " + data.getSegmentName(segmentId) + " because " + reason + ".", {
                'type': "explanation"
            });
            optly.client.preview.Logger.deleteReason(segmentId);
        }
        else {
            optly.client.preview.Logger.addEvent("Segment " + addOrRemove + " condition matches, " + addOrRemoveVerb + toOrFromPreposition + " segment " + data.getSegmentName(segmentId), {
                'type': "explanation"
            });
        }
    } : goog.nullFunction;
    /** @private */
    segmenter.changeHandlers = [];
    /**
    ...
    function isArrayLike(object) {
        return object && typeof object === "object" && object.length && typeof object.length === "number";
    }
    ...
    function isNumberLike(object) {
        return (typeof object === "number" || typeof object === "string") && Number(object) == object;
    }
    /**
     * @param {Object} object
     * @return {Array.<string>}
    ...
    timer.flushLogs = function () {
        if (directive.shouldLog) {
            each(timer.eventQueue, function (event) {
                if (!event.logged && (!event.isVerbose || event.isVerbose === directive.shouldLogVerbosely)) {
                    var time = +event.date;
    ...
    cookie.set = function (name, value, age) {
        ...
        if (domain && domain === cookie.publicSuffix && domain !== cookie.guessedPublicSuffix) {
        ...
        if (cookieValue === value) {
            ...
            log("Cookie", "Setting %s on %s apparently failed (%s != %s)", name, domain, cookieValue, value);
            log("Cookie", "Setting %s on %s", name, hostname);
            cookie.setOnDomain(name, value, hostname, age);
            cookieValue = cookie.get(name);
            if (cookieValue === value) {
                log("Cookie", "Setting %s on %s worked; saving as new public suffix", name, hostname);
    ...
    cookie.guessPublicSuffix = function (hostname) {
        ...
        if (parts.length > 2 && parts[parts.length - 2] === "appspot" && last === "com") {
    ...
    /** @private */
    cookie.guessedPublicSuffix = "";
    /** @private */
    cookie.apiPublicSuffix = ""; // Set via API
    /** @private */
    cookie.publicSuffix = "";
    /** @private */
    cookie.queue = [];
    ...
    extend(optly.client.Facade.prototype, /** @lends {optly.client.Facade.prototype} */ {
        ...
        getCampaign: function () {
            return this.getQueryParameter('utm_campaign');
        }
        ...
        getQueryParameter: function (name) {
            return query.getValue(name, this.getAllQueryParameters());
        }
        , getCustomTags: function () {
            return api.customTags;
        }
    ...
    optly.Visitor.prototype.initialize = function () {
        ...
        each(segmenter.getSegmentsList(), goog.bind(function (segmentId) {
            ...
            if (segment && segment['audience_id']) {
                ...
                this.setInAudience(segment['audience_id'], true, {
                    updateSegment: false
                });
            }
            else if (segment && segment['dimension_id']) {
    ...
    optly.Visitor.prototype.isInAudience = function (audienceId) {
        ...
        var isPersistentAudience = this.persistentAudiences.hasOwnProperty(audienceId) || this.segmentationAudiences.hasOwnProperty(audienceId);
    ...
    optly.Visitor.getConditionGetter = function (dimensionType) {
            var getters = {
                ...
                , 'browser_version': goog.bind(condition.facade.getBrowser, condition.facade), // "code" intentionally omitted since there is no "static" way to determine the code result
    ...
    evaluator.doSteps = function () {
        ...
        if (shouldWait) {
            var waitInterval = evaluator.waitCount === 0 ? FIRST_EVALUATION_WAIT_INTERVAL : EVALUATION_WAIT_INTERVAL;
    ...
    evaluator.evaluateCode = function (code, variationId) {
        ...
        if (redirect.getRedirectURL(code)) {
            ...
            if (redirect.okToRedirect(code)) {
                ...
                timer.addEvent("Evaluator", "setting a redirect cookie" + (variationId ? " for variation: " + variationId : ""));
        ...
        try {
            ...
            if (!!redirect.getRedirectURL(code)) {
                ...
                var isStandardRedirect = redirect.redirectRegexParams.test(code) || redirect.redirectRegex.test(code) || redirect.redirectRegexLegacy.test(code);
    ...
    evaluator.bindTrackElement = function (elementSelector, eventName, options) {
        ...
        evaluator.alreadyBoundClickTrackingEvents[elementSelector][eventName] = "mousedown touchstart";
    };
    /**
     * @param {string=} code
     */
    ...
    evaluator.tellPreviewAboutExperiments = function (experimentIds) {
        ...
        for (var i = 0; i < experimentIds.length; i++) {
            ...
            if (reason) {
                optly.client.preview.Logger.addEvent("Not activating " + data.getExperimentString(experimentId) + " because " + reason + ".", {
                    "type": "explanation"
                });
                optly.client.preview.Logger.deleteReason(experimentId);
            }
            else {
                optly.client.preview.Logger.addEvent("Activating " + data.getExperimentString(experimentId) + ".", {
                    "type": "activation"
                    , "experimentId": experimentId
                });
            }
    ...
    /** @private */
    evaluator.alreadyBoundClickTrackingEvents = {};
    /** @private */
    evaluator.evaluatedExperimentIds = [];
    /** @private */
    evaluator.evaluateQueue = evaluator.evaluateQueue || []; // contains an array of experiment ids to evaluate
    /** @private */
    evaluator.delayDomExecuteMilliseconds = 0;
    /** @private */
    evaluator.isDocumentReady = false;
    /** @private */
    evaluator.steps = [];
    /** @private */
    evaluator.timeoutId = null;
    /** @private */
    evaluator.waitCount = 0;
    /**
    ...
    optly.timeAndDayInterval.serialize = function (data) {
        return (data.start_time + optly.timeAndDayInterval.DELIM + data.end_time + optly.timeAndDayInterval.DELIM + data.days.join(optly.timeAndDayInterval.DAYS_DELIM));
    };
    /**
    ...
    optly.timeAndDayInterval.test = function (intervalStr, currentDate) {
            ...
            return (currentTime >= startTime && currentTime <= endTime && $.inArray(currentDay, interval.days) !== -1);
        }
    ...
    condition.test = function (conditions, objectId, objectType) {
        ...
        each(conditions, function (condition) {
            ...
            if (isExperiment && condition['only_first_time'] && plan.containsExperiment(objectId) && !optly.client.preview.ENABLED) {
                ...
                var verboseReason = "the visitor " + passVerb + " a " + type + " targeting condition " + " when it needed to " + shouldPassVerb;
    ...
    condition.testBrowsers = function (cond) {
        ...
        return any(cond['values'], function (targetedBrowser) {
            return condition.testBrowser(browser, {
                'value': targetedBrowser
            });
        });
    };
    /**
    ...
    condition.testBrowser = user.LEGACY_BROWSER ? function (browser, cond) {
        ...
        if (mobileId && mobileId !== "unknown") {
            ...
            return targetedBrowser === "mobile" || targetedBrowser === mobileId;
        }
    ...
    condition.testLanguages = function (cond) {
        ...
        return any(cond['values'], function (value) {
            return condition.testLanguage(browserLanguage, {
                'value': value
            });
        });
    };
    ...
    condition.testLocations = function (cond) {
        ...
        return any(cond['values'], function (value) {
            return condition.testLocation(userLocation, {
                'value': value
            });
        });
    };
    ...
    condition.testLocation = function (userLocation, cond) {
        ...
        case 1:
            ...
            break;
        case 2:
            if (user.formatGeoField(userLocation['region']) === region && user.formatGeoField(userLocation['country']) === country) {
                return true;
            }
            break;
        case 3:
            if (user.formatGeoField(userLocation['city']) === city && (user.formatGeoField(userLocation['region']) === region || "" === region) && user.formatGeoField(userLocation['country']) === country) {
                return true;
            }
            break;
        case 4:
    ...
    condition.testPlatform = function (platform, cond) {
        var targetedPlatform = cond['value'], // Split platform name from version.
        ...
        if (targetedPlatformName === platform.id) {
            ...
            if (targetedOSVersion.length > 1) {
                ...
                return condition.compareVersion(platform.version, targetedOSVersion[0]) >= 0 && condition.compareVersion(platform.version, targetedOSVersion[1]) <= 0;
            }
            else {
                // matchVersion format
    ...
    condition.testQueries = function (cond) {
        ...
        return any(cond['values'], function (queryCond) {
            ...
            return condition.testQueryParam(condition.facade.getQueryParameter(key), {
                'value': queryCond['value']
            });
        });
    };
    /**
  ...
  "www\."
    ...
    optly.taint.createDeconstructingFunction = function (originalFunction) {
        return function (object) {
            var shouldDeconstruct = typeof object === "object" && optly.taint.isObjectPrototypeTainted();
    ...
    optly.taint.createCleanJQueryEach = function (originalFunction, $) {
        ...
        return function (object, callback, args) {
            var shouldClean = (object.length === undefined || $.isFunction(object)) && optly.taint.isObjectPrototypeTainted();
            if (!shouldClean) {
    ...
    optly.taint.createCleanJQueryInit = function (originalFunction, $) {
        ...
        return function (selector, context, rootJQuery) {
            ...
            var shouldClean = typeof selector === "string" && context && $.type(context) === "object" && optly.taint.isObjectPrototypeTainted();
            if (!shouldClean) {
    ...
    plan.addVariation = function (variationId, source, shouldEvaluate, shouldReplace) {
        ...
        var added = false;
        var experimentId = data.getVariationExperimentId(variationId);
        if (experimentId && (shouldReplace || !plan.containsExperiment(experimentId))) {
            added = true;
            ...
            timer.addEvent("Plan", "Added experiment " + experimentId + " and variation id " + variationId + " to the plan, source is " + source, true);
        }
        return added;
    };
    /**
    ...
    plan.containsExperiment = function (experimentId) {
        return experimentId in plan.ignoredExperimentIds || experimentId in plan.variationExperimentIds;
    };
    /**
    ...
    plan.getSteps = function (experimentIds) {
        ...
        each(experimentIds, function (experimentId) {
            plan.getExperimentSteps(experimentId, eventSteps, globalSteps, forcedSteps, safeSteps);
        });
        each(variationIds, function (variationId) {
    ...
    plan.log = function () {
        if (directive.shouldLog) {
            each(plan.ignoredExperimentIds, function (experimentId) {
                var experimentName = data.getExperimentName(experimentId);
                timer.addEvent("Plan", "Ignore experiment '" + experimentName + "' (" + experimentId + ")");
            });
            each(plan.variations, function (variation, index) {
                ...
                timer.addEvent("Plan", data.getExperimentString(experimentId) + " in variation \"" + variationName + "\" (" + variation.id + ")");
            });
        }
    };
    plan.readCookie = function () {
        ...
        each(bucketMap, function (experimentId, variationId) {
            ...
            if (sectionIds.length > 1 && variationId.indexOf(VARIATION_ID_DELIMITER) === -1) {
        ...
        if (containsString(code, "_optimizely_redirect")) {
            ...
            while (codeLines.length > 0) {
                ...
                if (codeLine) {
                    ...
                    else if (containsRegEx(codeLine, /_optimizely_evaluate\s{0,9}=\s{0,9}safe/i) || containsRegEx(codeLine, /_optimizely_evaluate\s{0,9}=\s{0,9}end_force/i)) {
                        immediate = false;
                        continue;
                    }
    ...
    plan.commentRegex = /^(?:\s|(?:\/\/.*|\/\*(?:[^*]|\*(?!\/))*\*\/))*$/;
    /** @private */
    plan.jQueryExpression = /^\$j?\((['"].+?['"]|document)\)\..+;(?:\s|(?:\/\/.*|\/\*(?:[^*]|\*(?!\/))*\*\/))*$/;
    /** @private */
    plan.variationExperimentIds = {};
    /** @private */
    plan.variations = [];
...
optimizelyCode({
    ...
    , "version": "master-2650.396480867360816154"
    , "admin_account_id": 199382811
    , "blacklisted_experiments": [7503747712, 7565311935, 4961111812, 3227090433, 5330854280, 5179431308, 6486550291, 6789520533, 5926952219, 6430521862, 5974140203, 7551323056, 4933611455, 5014891329, 657512648, 6542271817, 5018371407, 7566681040, 7475181792, 6432224490, 623530862, 4953931508, 7464602997, 3202870392, 6811470456, 5969820030]
